...
//@TeleOp
public class FreightPIDCalibration extends OpMode {
    ...
    private int getModeIndex(){
        if(mode == FreightPIDCalibration.CalibrationMode.X) return 0;
        if(mode == FreightPIDCalibration.CalibrationMode.Y) return 1;
        if(mode == FreightPIDCalibration.CalibrationMode.PHI) return 2;
        else return -1;
    }
